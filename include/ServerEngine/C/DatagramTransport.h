#ifndef SERVERENGINE_DATAGRAM_TRANSPORT_H
#define SERVERENGINE_DATAGRAM_TRANSPORT_H

/* Optional datagram message transport, shared by native clients and servers.
...
#define SE_DATAGRAM_ABI_VERSION UINT32_C(0x00010000)
#define SE_DATAGRAM_UNRELIABLE UINT32_C(0)
#define SE_DATAGRAM_RELIABLE_ORDERED UINT32_C(1)
#define SE_DATAGRAM_CONNECTED UINT32_C(1)
#define SE_DATAGRAM_MESSAGE UINT32_C(2)
#define SE_DATAGRAM_DISCONNECTED UINT32_C(3)
#define SE_DATAGRAM_OVERFLOW UINT32_C(4)
 ...
 * use only in a trusted/protected network. It does not authenticate application users,
...
#define SE_DATAGRAM_ALLOW_REMOTE_UNAUTHENTICATED UINT32_C(1)

typedef uint64_t se_datagram_handle;

#pragma pack(push, 8)
typedef struct se_datagram_options {
...
typedef struct se_datagram_event {
    uint32_t struct_size;
    uint32_t abi_version;
    uint32_t kind;
    uint32_t delivery;
    uint64_t peer_id; /* Engine ID, scoped to this endpoint, never an application user ID. */
...
SE_API uint32_t SE_CALL se_datagram_get_abi_version(void);
SE_API void SE_CALL se_datagram_options_init(se_datagram_options* options);
SE_API void SE_CALL se_datagram_event_init(se_datagram_event* event);
/* Feature disabled: create returns SE_NOT_SUPPORTED and clears output. */
SE_API se_status SE_CALL se_datagram_create(const se_datagram_options* options,
    se_datagram_handle* endpoint, se_error* error);
/* One listener per endpoint; numeric IP only, port 1..65535. */
SE_API se_status SE_CALL se_datagram_listen(se_datagram_handle endpoint,
...
SE_API se_status SE_CALL se_datagram_connect(se_datagram_handle endpoint,
 ...
 * UNRELIABLE may be lost, duplicated or reordered: add application sequence IDs
...
SE_API se_status SE_CALL se_datagram_send(se_datagram_handle endpoint, uint64_t peer_id,
...
SE_API se_status SE_CALL se_datagram_poll(se_datagram_handle endpoint,
    se_datagram_event* event, void* payload, uint32_t capacity, uint32_t timeout_ms,
    se_error* error);
SE_API se_status SE_CALL se_datagram_disconnect(se_datagram_handle endpoint,
...
SE_API se_status SE_CALL se_datagram_destroy(se_datagram_handle endpoint, se_error* error);
