...
struct Endpoint {
    se_datagram_handle handle{};
    ~Endpoint() { if (handle) se_datagram_destroy(handle, nullptr); }
    Endpoint() = default;
...
bool poll(Endpoint& endpoint, Event& event, std::uint32_t wait = 0)
{
    ...
    const auto status = se_datagram_poll(endpoint.handle, &event.metadata, event.payload.data(),
        static_cast<std::uint32_t>(event.payload.size()), wait, nullptr);
    if (status == SE_TIMEOUT) return false;
    success(status, "poll loopback");
    check(event.metadata.kind != SE_DATAGRAM_OVERFLOW, "loopback event queue overflowed");
    return true;
}
...
std::uint32_t listen(Endpoint& server)
{
    ...
    for (std::uint32_t attempt = 0; attempt < 64; ++attempt) {
        ...
        if (se_datagram_listen(server.handle, "127.0.0.1", port, nullptr) == SE_OK) {
            check(se_datagram_listen(server.handle, "127.0.0.1", port, nullptr) == SE_INVALID_STATE,
...
std::uint64_t await_connection(Endpoint& server, Endpoint& client, std::uint64_t client_peer)
{
    ...
    while (Clock::now() < deadline && (!server_peer || !client_connected)) {
        ...
        if (poll(server, event)) {
            check(event.metadata.kind == SE_DATAGRAM_CONNECTED, "server expects connected event");
            server_peer = event.metadata.peer_id;
        }
        if (poll(client, event, 5)) {
            check(event.metadata.kind == SE_DATAGRAM_CONNECTED && event.metadata.peer_id == client_peer,
...
void buffer_retry(Endpoint& server, Endpoint& client, std::uint64_t server_peer, std::uint64_t client_peer)
{
    ...
    success(se_datagram_send(client.handle, client_peer, SE_DATAGRAM_RELIABLE_ORDERED,
    ...
    const auto deadline = Clock::now() + WaitLimit;
    se_datagram_event probe;
    bool found = false;
    while (Clock::now() < deadline) {
        Event ignored;
        check(!poll(client, ignored, 5), "unexpected client event during binary transfer");
        se_datagram_event_init(&probe);
        const auto status = se_datagram_poll(server.handle, &probe, nullptr, 0, 0, nullptr);
        if (status == SE_TIMEOUT) continue;
        check(status == SE_BUFFER_TOO_SMALL && probe.kind == SE_DATAGRAM_MESSAGE
            && probe.peer_id == server_peer && probe.delivery == SE_DATAGRAM_RELIABLE_ORDERED
    ...
    success(se_datagram_send(server.handle, server_peer, SE_DATAGRAM_RELIABLE_ORDERED, "PONG", 4, nullptr), "send reply");
    ...
    while (Clock::now() < reply_deadline && !reply) {
        ...
        if (poll(client, event, 5)) {
            check(event.metadata.kind == SE_DATAGRAM_MESSAGE && event.metadata.delivery == SE_DATAGRAM_RELIABLE_ORDERED
...
void unreliable_reception(Endpoint& server, Endpoint& client, std::uint64_t client_peer)
{
    ...
    while (Clock::now() < deadline && !received) {
        if (Clock::now() >= next_send) {
            ...
            success(se_datagram_send(client.handle, client_peer, SE_DATAGRAM_UNRELIABLE,
        ...
        if (poll(server, event)) {
            check(event.metadata.kind == SE_DATAGRAM_MESSAGE && event.metadata.delivery == SE_DATAGRAM_UNRELIABLE
...
{
    success(se_datagram_disconnect(client.handle, client_peer, nullptr), "disconnect client peer");
    const auto stale = se_datagram_send(client.handle, client_peer, SE_DATAGRAM_RELIABLE_ORDERED, "x", 1, nullptr);
    ...
    while (Clock::now() < deadline && !server_closed) {
        ...
        if (poll(server, event) && event.metadata.kind == SE_DATAGRAM_DISCONNECTED) {
    ...
    success(se_datagram_create(&options, &waiting.handle, nullptr), "create waiting endpoint");
    ...
    auto waiter = std::async(std::launch::async, [&] {
        ...
        entering.set_value();
        return se_datagram_poll(waiting_handle, &event, nullptr, 0, 30000, nullptr);
    });
    entered.wait();
    success(se_datagram_destroy(waiting.handle, nullptr), "destroy wakes waiter");
    waiting.handle = 0;
...
int main()
{
    try {
        ...
        const auto capability = se_datagram_create(&options, &server.handle, nullptr);
        if (capability == SE_NOT_SUPPORTED) {
            check(server.handle == 0, "disabled create clears handle");
            std::cout << "SKIP datagram transport loopback: feature disabled\n";
            return 77;
        }
        success(capability, "create server endpoint");
        success(se_datagram_create(&options, &client.handle, nullptr), "create client endpoint");
        const auto port = listen(server);
        std::uint64_t client_peer{};
        success(se_datagram_connect(client.handle, "127.0.0.1", port, &client_peer, nullptr), "connect client");
        ...
        std::cout << "PASS datagram transport loopback, ordered stream, caller buffers and lifecycle\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "FAIL datagram transport loopback: " << error.what() << '\n';
        return 1;
    }
}
