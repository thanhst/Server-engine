...
bool valid_options(const se_game_options* options) noexcept
{
    if (options == nullptr || options->struct_size != sizeof(*options)
        ...
        || (options->flags & ~SE_GAME_ALLOW_REMOTE_UNAUTHENTICATED) != 0
    ...
    if (options->max_peers == 0 || options->max_peers > 1024
        ...
        || options->max_event_queue_bytes < sizeof(se_game_event) + options->max_message_bytes
...
se_game_event empty_event() noexcept
{
    ...
    value.abi_version = SE_GAME_ABI_VERSION;
    return value;
}
